...
class MSHFeatureExtractor(nn.Module):
    ...
    该特征提取器中只启用多尺度感知模块，通过多尺度卷积提取帕金森病信号中的关键特征
    """
    ...
    def __init__(self, in_channels=132, out_channels=132, tremor_freq_band=(3, 7), fs=100):
        ...
        self.fusion = nn.Sequential(
            nn.Conv1d(32 * 3, 64, kernel_size=1),  # 3个卷积层，每个32通道
            nn.BatchNorm1d(64),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        # 拼接特征
        multi_scale_features = torch.cat([feat_small, feat_med, feat_large], dim=1)
